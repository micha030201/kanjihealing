...
with open('topokanji/dependencies/1-to-N.json') as f:
...
def decompose(p):
    return [equiv(c) for c in decomposition[p]]
...
if __name__ == '__main__':
    ...
    for e in elements:
        if e.name not in decomposition:
            continue
        elements_kanjivg = list(sorted([
            str(c.name)
            for c in e.children
            if type(c) is LogicalElement]))
        elements_chise = list(sorted(list(decomposition.get(e.name, []))))
